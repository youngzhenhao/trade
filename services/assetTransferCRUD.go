...
// AssetTransferProcessed

func CreateAssetTransferProcessed(assetTransferProcessed *models.AssetTransferProcessed) error {
	return middleware.DB.Create(assetTransferProcessed).Error
}

func CreateAssetTransferProcessedSlice(assetTransferProcessedSlice *[]models.AssetTransferProcessed) error {
	return middleware.DB.Create(assetTransferProcessedSlice).Error
}

func ReadAllAssetTransferProcessedSlice() (*[]models.AssetTransferProcessed, error) {
...
func ReadAssetTransferProcessed(id uint) (*models.AssetTransferProcessed, error) {
	var assetTransferProcessed models.AssetTransferProcessed
	err := middleware.DB.First(&assetTransferProcessed, id).Error
	return &assetTransferProcessed, err
}

func ReadAssetTransferProcessedSliceByUserId(userId int) (*[]models.AssetTransferProcessed, error) {
	var assetTransferProcessedSlice []models.AssetTransferProcessed
	err := middleware.DB.Where("user_id = ? AND status = ?", userId, 1).Find(&assetTransferProcessedSlice).Error
	return &assetTransferProcessedSlice, err
}

func ReadAssetTransferProcessedByTxid(txid string) (*models.AssetTransferProcessed, error) {
	var assetTransferProcessed models.AssetTransferProcessed
	err := middleware.DB.Where("txid = ? AND status = ?", txid, 1).First(&assetTransferProcessed).Error
	return &assetTransferProcessed, err
}

func ReadAssetTransferProcessedByAnchorTxHash(anchorTxHash string) (*models.AssetTransferProcessed, error) {
	var assetTransferProcessed models.AssetTransferProcessed
	err := middleware.DB.Where("anchor_tx_hash = ? AND status = ?", anchorTxHash, 1).First(&assetTransferProcessed).Error
	return &assetTransferProcessed, err
}

func UpdateAssetTransferProcessed(assetTransferProcessed *models.AssetTransferProcessed) error {
	return middleware.DB.Save(assetTransferProcessed).Error
}

func UpdateAssetTransferProcessedSlice(assetTransferProcessedSlice *[]models.AssetTransferProcessed) error {
